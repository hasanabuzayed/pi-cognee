...
const SERVER_ENV_FILE_DENYLIST = /^(PATH|HOME|SHELL|USER|PYTHON\w*|LD_.+|DYLD_.+)$/;

const VERSION_PROBE_SCRIPT = "import importlib.metadata as m; print(m.version('cognee'))";
...
export function logBootstrapEvent(event: Record<string, unknown>, stateRoot?: string): void {
  try {
    const paths = bootstrapPaths(stateRoot);
    mkdirSync(dirname(paths.bootstrapLog), { recursive: true });
    const maxBytes = envNumber("COGNEE_PLUGIN_LOG_MAX_BYTES", 20 * 1024 * 1024);
    try {
...
function writeServerPidfile(port: number, pid: number, stateRoot?: string): void {
  try {
    const paths = bootstrapPaths(stateRoot);
    ...
    logBootstrapEvent({ event: "server_pidfile_write_failed", error: describeError(err) }, stateRoot);
  }
}
 ...
 */
async function liveServerPid(port: number, stateRoot?: string): Promise<number> {
  ...
  if (Number.isInteger(pid) && pid > 0 && pidAlive(pid) && (await pidLooksLikeServer(pid))) {
    return pid;
  }
  try {
    unlinkSync(path);
...
export function serverPidfileStatus(port: number, stateRoot?: string): string {
  const path = bootstrapPaths(stateRoot).pidFile(port);
  try {
    const record = JSON.parse(readFileSync(path, "utf8")) as ServerPidRecord;
...
async function httpHealthOk(baseUrl: string, timeoutMs = HEALTH_PROBE_TIMEOUT_MS): Promise<boolean> {
  return (await httpHealth(baseUrl, timeoutMs)).ok;
}

/** Loopback TCP handshake: "listening" | "refused" | "no_verdict". */
function tcpProbe(host: string, port: number, timeoutMs = TCP_PROBE_TIMEOUT_MS): Promise<string> {
  return new Promise((resolve) => {
    ...
    socket.once("error", (err) => {
      done((err as NodeJS.ErrnoException).code === "ECONNREFUSED" ? "refused" : "no_verdict");
    });
  });
}
...
export async function serverPresence(
  ...
  try {
    ...
    if (opts.confirmAbsent) {
      const delayMs =
        opts.reprobeDelayMs ?? envNumber("COGNEE_PRESENCE_REPROBE_DELAY", 3) * 1000;
      await sleep(delayMs);
      const retryHttp = await httpHealth(baseUrl, PRESENCE_REPROBE_TIMEOUT_MS);
      evidence.http_retry = retryHttp.ok ? "ready" : (retryHttp.status ?? retryHttp.error);
...
export async function acquireJsonLock(
  ...
  for (;;) {
    ...
    try {
      ...
      try {
        writeSync(fd, JSON.stringify({ owner: opts.owner, pid: process.pid, created_at: Date.now() / 1000 }));
        created = true;
      } finally {
        closeSync(fd);
      }
...
export function cogneeInstallSpec(env: EnvLookup): string {
  ...
  if ((env("VECTOR_DB_PROVIDER") ?? "").trim().toLowerCase() === "pgvector") add("postgres-binary");
  if ((env("GRAPH_DATABASE_PROVIDER") ?? "").trim().toLowerCase() === "neo4j") add("neo4j");
  if ((env("EMBEDDING_PROVIDER") ?? "").trim().toLowerCase() === "fastembed") add("fastembed");
  if ((env("LLM_PROVIDER") ?? "").trim().toLowerCase() === "ollama") add("ollama");
...
export async function installUv(stateRoot?: string, timeoutMs = UV_INSTALL_TIMEOUT_MS): Promise<string> {
  const paths = bootstrapPaths(stateRoot);
  try {
    await execFileAsync("sh", ["-c", "curl -LsSf https://astral.sh/uv/install.sh | sh"], {
      env: { ...process.env, UV_UNMANAGED_INSTALL: paths.uvDir },
      timeout: timeoutMs,
      windowsHide: true,
    });
  } catch (err) {
    logBootstrapEvent({ event: "uv_install_failed", error: describeError(err) }, stateRoot);
    return "";
  }
  const uv = findUv(stateRoot);
  if (!uv) logBootstrapEvent({ event: "uv_install_not_found_after_install" }, stateRoot);
  return uv;
}
...
export async function venvCogneeVersion(
  ...
  try {
    const { stdout } = await execFileAsync(venvPython, ["-c", VERSION_PROBE_SCRIPT], {
      timeout: timeoutMs,
      windowsHide: true,
    });
    return stdout.trim();
  } catch (err) {
    logBootstrapEvent({ event: "cognee_version_probe_failed", error: describeError(err) }, stateRoot);
    return "";
  }
}
...
export async function venvMissingExtras(
  ...
  const dists = extras.map((extra) => EXTRA_SENTINEL_DISTS[extra]).filter(Boolean);
  if (!dists.length) return [];
  try {
    const { stdout } = await execFileAsync(venvPython, ["-c", EXTRAS_PROBE_SCRIPT, ...dists], {
      timeout: timeoutMs,
      windowsHide: true,
    });
    const missingDists = new Set(stdout.split(/\s+/).filter(Boolean));
    return extras.filter((extra) => missingDists.has(EXTRA_SENTINEL_DISTS[extra]));
  } catch (err) {
    logBootstrapEvent({ event: "extras_probe_failed", error: describeError(err) });
    return [];
  }
}
...
function writeVenvReady(paths: BootstrapPaths, version: string): void {
  try {
    ...
    logBootstrapEvent({ event: "venv_ready_write_failed", error: describeError(err) }, paths.stateRoot);
  }
}
...
export async function ensureCogneeInstalled(
  ...
  const timeoutMs = opts.timeoutMs ?? envNumber("COGNEE_INSTALL_TIMEOUT", 600) * 1000;
  ...
  try {
    ...
    return { ok: false, error: `cannot create cognee data dirs: ${describeError(err)}` };
  }

  const spec = cogneeInstallSpec(env);
  const extras = requiredExtras(spec);
  const pythonPin = (envValue("COGNEE_PLUGIN_PYTHON") || DEFAULT_PYTHON_PIN).trim() || DEFAULT_PYTHON_PIN;
  ...
  if (lock === "probe-shortcut") {
    logBootstrapEvent(
      { event: "install_lock_lost_skip_at_pin", version: PINNED_COGNEE_VERSION, via: "wait_probe" },
      opts.stateRoot,
    );
  ...
  if (!lock) {
    ...
    const version = await venvCogneeVersion(paths.venvPython, VERSION_PROBE_TIMEOUT_MS, opts.stateRoot);
    const missing = await venvMissingExtras(paths.venvPython, extras);
    if (version === PINNED_COGNEE_VERSION && missing.length === 0) {
      logBootstrapEvent({ event: "install_lock_lost_skip_at_pin", version }, opts.stateRoot);
      return { ok: true, version, skippedAtPin: true };
    }
    logBootstrapEvent({ event: "install_lock_lost_best_effort", version }, opts.stateRoot);
  ...
  try {
    ...
    if (version === PINNED_COGNEE_VERSION) {
      ...
      if (missing.length === 0) {
        ...
        logBootstrapEvent(
          { event: "cognee_install_skipped_at_pin", version, spec }, opts.stateRoot,
        );
        return { ok: true, version, skippedAtPin: true };
      }
      logBootstrapEvent({ event: "cognee_install_missing_extras", missing }, opts.stateRoot);
    }

    // uv resolution: self-managed → PATH → install once.
    let uv = findUv(opts.stateRoot);
    if (!uv) {
      logBootstrapEvent({ event: "uv_not_found_installing" }, opts.stateRoot);
    ...
    if (!uv) {
      return {
        ok: false,
        error:
          "uv not found and automatic install failed — install uv (https://docs.astral.sh/uv/getting-started/installation/) and restart pi",
      };
    }

    const uvEnv = { ...process.env, UV_PYTHON_INSTALL_DIR: paths.uvPythonDir };
    try {
      if (!existsSync(paths.venvPython)) {
        await execFileAsync(uv, ["venv", paths.venvDir, "--python", pythonPin], {
          env: uvEnv,
          cwd: paths.stateRoot,
          timeout: timeoutMs,
          windowsHide: true,
      ...
      await execFileAsync(uv, ["pip", "install", "--upgrade", "--python", paths.venvPython, spec], {
        env: uvEnv,
        cwd: paths.stateRoot,
        timeout: timeoutMs,
        windowsHide: true,
      ...
      logBootstrapEvent({ event: "cognee_install_failed", via: "uv", spec, error: describeError(err) }, opts.stateRoot);
      return { ok: false, error: `uv install failed: ${describeError(err)}` };
    }

    const finalVersion = await venvCogneeVersion(paths.venvPython, VERSION_PROBE_TIMEOUT_MS, opts.stateRoot);
    if (!finalVersion) {
      return { ok: false, error: "venv unusable after install (cognee not importable)" };
    }
    writeVenvReady(paths, finalVersion);
    logBootstrapEvent({ event: "cognee_install_ready", version: finalVersion, spec }, opts.stateRoot);
 ...
 */
export function buildServerEnv(envFileValues: Record<string, string>): NodeJS.ProcessEnv {
...
function stdinFd(child: ChildProcess | undefined): number | undefined {
  const stdin = child?.stdin as { _handle?: { fd?: unknown } } | null | undefined;
...
function startConsolePump(
  ...
  try {
    ...
    logBootstrapEvent({ event: "console_pump_start_failed", error: describeError(err) }, stateRoot);
    return undefined;
  }
}
...
export async function ensureLocalServerRunning(
  ...
  const healthTimeoutMs =
    opts.healthTimeoutMs ?? envNumber("COGNEE_SERVER_BOOT_DEADLINE", 600) * 1000;
  ...
  const install = await ensureCogneeInstalled(env, { stateRoot: opts.stateRoot });
  if (!install.ok) {
    return { ok: false, adopted: false, error: `cognee runtime unavailable (${install.error ?? "install/upgrade failed"})` };
  }
  ...
  try {
    ...
    const pump = startConsolePump(consolePath, CONSOLE_LOG_CAP_BYTES, opts.stateRoot);
    const pumpFd = stdinFd(pump);
    if (pump && pumpFd === undefined) {
      pump.stdin?.destroy(); // EOF — the idle pump exits instead of lingering
      logBootstrapEvent(
        { event: "server_console_capture_unavailable", path: consolePath, error: "pump stdin fd unavailable" },
        opts.stateRoot,
      );
    }
    ...
    const child = spawn(
      ...
      ["-m", "uvicorn", "cognee.api.client:app", "--port", String(port)],
      {
        ...
        env: serverEnv,
        detached: true,
        stdio: pumpFd !== undefined ? ["ignore", pumpFd, pumpFd] : "ignore",
        windowsHide: true,
      },
    );
    ...
    if (typeof child.pid === "number" && child.pid > 0) {
      ...
      logBootstrapEvent(
        { event: "server_spawned", pid: child.pid, port, version: PINNED_COGNEE_VERSION },
        opts.stateRoot,
      );
    } else {
      logBootstrapEvent({ event: "server_spawn_failed_no_pid", error: spawnErrorMessage }, opts.stateRoot);
    }
    ...
    for (;;) {
      ...
      if (await httpHealthOk(baseUrl)) {
        logBootstrapEvent({ event: "server_healthy", port, pid: child.pid ?? null }, opts.stateRoot);
      ...
      if (spawnErrorMessage || child.exitCode !== null) {
        ...
        while (Date.now() < readoptDeadline) {
          if (await httpHealthOk(baseUrl, 500)) return { ok: true, adopted: true };
          await sleep(250);
        }
        ...
        logBootstrapEvent(
          { event: "server_exited_before_healthy", reason, console_tail: tail.slice(-600) },
          opts.stateRoot,
        );
        return {
          ok: false,
      ...
      if (Date.now() >= deadline) {
        const tail = consoleTail(consolePath, 4000);
        logBootstrapEvent({ event: "server_health_deadline", port, console_tail: tail.slice(-600) }, opts.stateRoot);
        return {
          ok: false,
          adopted: false,
          error:
...
async function requireAbsent(
  ...
  const presence = await serverPresence(baseUrl, { confirmAbsent: stage === "pre_install", stateRoot });
  if (presence.verdict === "ready") return { ok: true, adopted: true };
  if (presence.verdict === "absent") return null;
  logBootstrapEvent(
    { event: "boot_refused_server_present", stage, verdict: presence.verdict, ...presence.evidence },
    stateRoot,
  );
  return {
    ok: false,
