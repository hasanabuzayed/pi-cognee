...
import { CogneeError, piStateDir, type SessionQaRow, type SessionTraceRow } from "./client";
...
function readBridgeFile(sessionId: string): BridgeFile {
  try {
    const raw = JSON.parse(readFileSync(bridgePath(sessionId), "utf8")) as BridgeFile;
...
export function stableStringify(value: unknown): string {
  try {
    const out = JSON.stringify(value, (_k, v) => {
      if (typeof v === "function" || typeof v === "bigint") return String(v);
...
export function serverFingerprints(qas: SessionQaRow[], traces: SessionTraceRow[] = []): Set<string> {
...
export function bridgeBackoffMs(failCount: number): number {
  ...
  return Math.min(DRAIN_BACKOFF_BASE_MS * 2 ** (failCount - 1), DRAIN_BACKOFF_MAX_MS);
}

/** True while the session's file sits inside its failure backoff window. */
export function bridgeBackoffOpen(sessionId: string, nowMs: number = Date.now()): boolean {
  const file = readBridgeFile(sessionId);
...
export function oldestPendingBridgeHead(
  ...
  let best: { sessionId: string; entry: SpilledEntry; at: number } | undefined;
  for (const name of dir) {
    if (!name.endsWith(".json")) continue;
    try {
      const file = JSON.parse(readFileSync(join(bridgeDir(), name), "utf8")) as BridgeFile;
      ...
      if (excludeSessionId && head.sessionId === excludeSessionId) continue;
      const at = Number(head._buffered_at) || 0;
      if (!best || at < best.at) best = { sessionId: head.sessionId, entry: head, at };
    } catch {
...
export function sweepOldBridgeFiles(maxAgeMs: number = 7 * 24 * 60 * 60 * 1000): void {
  let dir: string[];
  try {
    dir = readdirSync(bridgeDir());
  } catch {
